...
export const handler: Handlers<BlogPost | null> = {
  async GET(_req, ctx) {
    ...
    const post={ ...data.snippet, tags: [...data.snippet.tags, 'haikel fazzani', 'tutorials', 'mobile developement', 'web app', 'network security', 'blog dev'] };
...
export default function SnippetPost({ data }: PageProps<BlogPost>) {
  return <>
    <Head>
      ...
      <meta property="og:title" content={data.title + '| Snippets'} />
    ...
    <main class="grid-2-1 p-0">
      ...
      <div class="border-left p-2 overflow">
        <div class="d-flex flex-wrap border-bottom pb-1 mb-1">
          {data.tags.slice(0,3).map((tag, i) => <a href={"/snippets?tag=" + tag} class="tag cp mr-2 mb-1" key={i}>{tag}</a>)}
        </div>
